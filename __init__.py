...
bot = TelegramClient("WintunelingVPN", "8423277151", "AAFonPXB47J6EU7x0urwH-wI59xlo0MebWE").start(bot_token=BOT_TOKEN)
...
try:
    ...
    if count == 0:
        c.execute("INSERT INTO server (button_name, host, password) VALUES (?, ?, ?)",
                  ('SG', 'cella.windratuneup.my.id', 'Bapaksaur123@.e'))
...
async def process_user_balance_vmess(event, user_id):
    ...
    try:
        ...
        if user_data:
            ...
            if saldo >= 15000:  # Hanya lanjutkan jika saldo mencukupi
                saldo -= 15000
                cursor.execute("UPDATE user SET saldo = ? WHERE user_id = ?", (saldo, user_id))
                connection.commit()
